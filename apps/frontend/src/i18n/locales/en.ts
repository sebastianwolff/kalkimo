...
export default {
  ...
  property: {
    ...
    marketReference: {
      ...
      hint: 'Optional. Enables comparison of purchase price against regional market value.',
      pricePerSqm: 'Regional Price per sqm',
      fairValue: 'Calculated Market Value',
      hintEmpty: 'Enter regional price per sqm to calculate market value'
    }
  },
  ...
  costs: {
    ...
    maintenanceReserve: {
      ...
      monthlyAmount: 'Reserve/Month',
      annualIncrease: 'Annual Increase',
      hint: 'Reduces cashflow but is not immediately tax-deductible.',
      yearly: 'Reserve/Year',
    },
    totalWithReserve: 'Total incl. Reserve'
  },
  ...
  capex: {
    ...
    oneTime: {
      toggle: 'One-Time Investment',
    },
    recurring: {
      toggle: 'Recurring Measure',
      intervalPercent: 'Interval (% of Cycle)',
      costPercent: 'Cost (% of Renewal Cost)',
      cycleExtension: 'Cycle Extension (%)',
      calculatedInterval: 'Interval: {years} years',
      calculatedCost: 'Cost per occurrence: {cost}',
      occurrences: '{count}× in period: {years}',
      noOccurrences: 'No occurrences in analysis period',
    },
    taxHint: {
      beforePurchase: 'Before purchase: Classified as acquisition cost (§255 HGB).',
      rule15Exceeded: '15% threshold exceeded: Maintenance costs ({total}) within 3 years exceed 15% of building value ({threshold}) → acquisition-related costs (§6 para. 1 No. 1a EStG).',
      within3Years: 'Within 3-year post-acquisition period. 15% building value threshold: {threshold} (remaining: {remaining}).',
      afterThreeYears: 'Maintenance expense: Immediately deductible or §82b distribution (2–5 years).',
    }
  },
  ...
  summary: {
    ...
    tax: {
      ...
      totalMaintenanceReserve: 'Maintenance Reserve (not deductible)',
      rule15: '15% Rule',
      ...
      bridge: {
        ...
        reserve: 'Reserve (non-deductible)',
    ...
    propertyValue: {
      ...
      subtitle: 'Estimated market value considering condition, investments and market development',
      purchasePrice: 'Purchase Price',
      marketValue: 'Market Value (price development only)',
      improvementUplift: 'Value from Investments',
      conditionFactor: 'Condition Factor',
      estimatedValue: 'Estimated Value',
      initialCondition: 'Condition Factor at Purchase',
      improvementFactor: 'Investment Value Transfer',
      scenario: {
      ...
      meanReversionRow: 'Market Convergence',
      regionalPrice: 'Regional Price/sqm',
      fairMarketValue: 'Fair Market Value',
      purchaseVsMarket: 'Purchase vs. Market',
      assessment: {
        below: 'Below Market',
        at: 'At Market',
        above: 'Above Market',
      },
      endConditionFactor: 'Condition Factor at End',
      explanationTitle: 'Explanation',
      driver: {
        initialCondition: 'Building constructed {constructionYear}, condition {condition}, {componentCount} components tracked. Initial factor: {factor}%.',
        overdueComponents: '{count} components overdue ({names}) — average {avgOverdueYears} years past renewal cycle. This significantly impacts the condition factor.',
        degradation: 'Condition factor declines from {startFactor}% to {endFactor}% over {years} years (−{totalDecline} percentage points) due to natural aging.',
        componentDeterioration: '{uncoveredCount} components become due during holding period without matching CapEx. Estimated deterioration: {uncoveredAmount} €.',
        investments: '{measureCount} measures planned ({totalAmount} €). Value contribution: {valueUplift} € (70% transfer). Condition improvement: +{conditionBoost} percentage points.',
        marketAppreciation: 'Market development {rate}% p.a. over {years} years yields +{appreciationPercent}% price appreciation ({appreciationAmount} €).',
        meanReversion: 'Purchase price {gapPercent}% {assessment} regional market value — {direction} of {adjustmentAmount} € (half-life 7 years).',
        summary: 'Estimated value after {years} years (base scenario): {finalValue} € ({changeDirection}{changePercent}% or {changeDirection}{changeAbsolute} € vs. purchase price).',
      },
      componentDeterioration: {
        title: 'Component Condition Development',
        subtitle: 'Value deterioration from renewals becoming due during holding period',
        component: 'Component',
        age: 'Age (End)',
        cycleYears: 'Cycle',
        dueYear: 'Due',
        ...
        statusAtEnd: 'Status',
        totalUncovered: 'Uncovered Deterioration',
        totalCovered: 'Covered by CapEx',
        statusLabels: {
          OK: 'OK',
          Overdue: 'Overdue',
          OverdueAtPurchase: 'Overdue at Purchase',
          Renewed: 'Renewed',
        },
        pricedIn: 'priced in',
    ...
    exit: {
      title: 'Exit Analysis',
      subtitle: 'Total return over holding period including disposal',
      holdingPeriod: 'Holding Period',
      years: 'years',
      speculationPeriod: 'Speculation Period (§23 EStG)',
      withinPeriod: 'Within 10-year period',
      outsidePeriod: 'Outside period (tax-free)',
      pnlTitle: 'Profit & Loss (Holding Period)',
      totalGrossIncome: 'Total Rental Income',
      totalOperatingCosts: 'Operating Costs',
      totalDebtService: 'Debt Service',
      totalCapex: 'Capital Expenditure',
      totalTaxPaid: 'Income Tax',
      totalMaintenanceReserve: 'Maintenance Reserve',
      finalReserveBalance: 'Reserve Balance at Exit',
      netCashflow: 'Net Cashflow (after Tax)',
      scenarioTitle: 'Exit Scenarios',
      scenario: {
        conservative: 'Conservative',
        base: 'Base',
        optimistic: 'Optimistic',
